    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float3 bitangent = cross(normal, tangent) * input.tangentWS.w; // handedness 적용
                float3x3 TBN = float3x3(tangent, bitangent, normal);
                float3 viewDirTS = mul(normalize(input.viewDirWS), TBN);
                float denom = max(abs(viewDirTS.z), 0.01); // 0으로 나누는 것 방지
                float2 parallaxOffset = (viewDirTS.xy / denom) * ((height - 0.5) * _HeightScale);
                uv += parallaxOffset;
                #endif
                ...
                #else
                N = normalize(input.normalWS.xyz);
                #endif
                ...
                float3 emission = emissionMap.rgb * _EmissionColor.rgb * _EmissionColor.a * _EmissionIntensity;
