...
public class CSVReader
{
    ...
    public void get(int row, ref ObjectiveType val)
    {
        string s = _arr_grid[cur_col, row];
        ++cur_col;
        if ((s == null) || (s == ""))
        {
            val = ObjectiveType.Interact;
            return;
        }
        val = (ObjectiveType)System.Convert.ToInt32(s);
    }
    ...
    public void get(int row, ref List<int> vals)
    {
        vals = new List<int>();
        ...
        for (int i = 0; i < arr.Length; ++i)
        {
            if (string.IsNullOrEmpty(arr[i]))
                continue;
            vals.Add((int)System.Convert.ToInt32(arr[i]));
        }
    }
