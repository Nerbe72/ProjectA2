...
public class TableScroll : TableBase
{
    ...
    protected bool Read(CSVReader _Reader, Info _Info, int _Row, int _Col)
    {
        ...
        _Info.IngredientIDs = new List<string>();

        _Reader.get(_Row, ref _Info.ID);
        _Reader.get(_Row, ref _Info.CreationItemID);
        _Reader.get(_Row, ref _Info.Price);
        _Reader.get(_Row, ref _Info.IngredientIDs, 10);
